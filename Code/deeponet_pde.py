...
def run(problem, system, space, T, m, nn, net, lr, epochs, num_train, num_test):
   ...
   # )
    ...
    #print(f'Step: ', losshistory.steps, ' Training Loss: ', losshistory.loss_train, ' Testing Loss: ', losshistory.loss_test)
    ...
    plt.savefig('./figures/plot_loss')
    dde.postprocessing.plot_best_state(train_state)
    plt.savefig('./figures/best_state')
    dde.postprocessing.save_loss_history(losshistory, fname='loss_file')
    ...
    tests = [
        (lambda x: x, "x_dr.dat"),
        (lambda x: np.sin(np.pi * x), "sinx_dr.dat"),
        (lambda x: np.sin(2 * np.pi * x), "sin2x_dr.dat"),
        (lambda x: x * np.sin(2 * np.pi * x), "xsin2x_dr.dat"),
    ]
...
def main():
    ...
    nn = "opnn"
    activation = "relu"
    initializer = "He normal"  # "He normal" or "Glorot normal"
    stacked=True
    unstacked=False
